defmodule Rumbl.AuthTest do
  ...
  test "login with passwword mismatch", %{conn: conn} do
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Rumbl.User{}) |> Auth.authenticate_user([])
    refute conn.halted
  end
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Rumbl.User{id: 124})
      |> send_resp(:ok, "")

    next_conn = get(login_conn, "/")
    assert get_session(next_conn, :user_id) == 124
  end
end
